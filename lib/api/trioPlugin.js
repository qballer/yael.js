...
var trio = function (){
	...
	that.attach = function (options){
		...
		if (userDivider !== null && userAssembler !== null){
			//divider setup
			that.divider = this.divider = new dividerWrapper(userDivider);
			...
			//assembler setUp
			that.assembler = this.assembler = new assemblerWrapper(userAssembler);
			this.assembler.grandHalt = options.grandHalt;
			this.assembler.takeResultFromNetwork = options.takeResultFromNetwork;
			this.assembler.errorHalt = options.errorHalt;
			this.assembler.pathToFolder = options.pathToFolder;
		}else{
	...
	that.init = function(callback){
		debugger;
		that.divider.divider.getSliceZero();
		that.assembler.assembler.init();
