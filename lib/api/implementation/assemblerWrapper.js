...
var AssemMessageBox = function (assembler){
	...
	that.stopOnError = function (errorMsg){
		...
		that.context.giveResultToControl(errorMsg, noPathToFile);
	};
	...
	that.finalResultCB = function (PathToResultFile){
		...
		that.context.giveResultToControl(noMsg,PathToResultFile);
	};
	...
	that.callFinalResult = function(){
		that.context.closeNetwork();
		that.assembler.finalResult();
	};
