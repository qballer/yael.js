...
var yael = function(){
	...
	var passSliceToAssembler = function(result,json){
		...
		console.log('your mommayour mommayour mommayour mommayour momma');
	};
	...
	that.giveResult = function (slice){
		...
		console.log('giveResult slice.id' + slice.ID);
		getSliceByAction(function (sl){
			if(sl !== null){
				...
				console.log("sl = null!!!:   NOTICE ME! : " + sl);
			}
		}, slice.ID,true, "GET");
	};
	that.flushDB = function(){
		that.client[freshID].send_command("FLUSHDB",function (err,reply){
			console.log("reply on flashDB: " +reply );
		});
		that.client[sendID].send_command("FLUSHDB",function (err,reply){
			console.log("reply on flashDB: "+reply );
		});
	};
