	...
	if (data.algos.length > 0){
		if (data.flag === false){
			...
			for (var i=0; i<data.algos.length; i++){
				...
				'<input type="button" name="resultButton" id="results'+i+'Button" value="Get Result" title="Click to get the algorithm output" onclick="getResult()" disabled="disabled"/>' +
				'</div>';
			}
		}else{
...
algoCompleted = function(algoIndex){
	...
	document.getElementById("results"+algoIndex+"Button").disabled = "";
}