	...
	if (data.algos.length > 0){
		if (data.flag === false){
			...
			for (var i=0; i<data.algos.length; i++){
				...
				'<input type="button" name="resultButton" id="resultsButton" value="Get Result" title="Click to get the algorithm output" onclick="getResult()" disabled="disabled"/>' +
				'</div>';
			}
		}else{
