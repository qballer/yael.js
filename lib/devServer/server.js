...
function start(route, routes) {
  ...
  storeResultSlice = function(object){
    ...
    for (var i =0 ; i < 16 ; i++){
      console.log("STORING SLICE: "+ResultSlice.buffer[i]);  
    }
  ...
  io.sockets.on('connection', function(socket) {
    ...
    socket.on('resultSlice', function(data) {
      console.log("IN SERVER.JS RECIVING RESULT SLICE: "+data);
      storeResultSlice(data);
      getNewSlice(function(slice){
        if (slice !== null){
          for (var i = 0 ; i < 16 ; i++){
            console.log("IN SERVER.JS SENDING SLICE: "+slice.buffer[i]);
          }
          socket.emit('nakedSlice', slice);
        }
        else
        ...
        function(data){
          ...
          trioContext.startPlugin(algos[algoIndex].algoPath, algoIndex, function(msg, path2File){
            ...
            if (path2File !== null){
              ...
              //toDo: emit event to activate getResult with path2file
              socket.emit('algoCompleted', path2file);
            }else{
