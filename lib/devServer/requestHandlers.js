...
var indexPage, noAlgoPage, adminPage, adminScript, contextScript, clientScript, nakedSliceScript, 
    logo, algoMessageBoxScript, file2Send;
...
/*fs.readFile('../../../../algo2.zip', function (err, data) {
    if (err) {
        throw err;
    }
    file2Send = data;
...
function sendResult(response, request) {
  if (server.getPath2ResultFile() !== null && server.getPath2ResultFile() !== 'undefined'){
    console.log(server.getPath2ResultFile());
    fileReader.readStaticFile(server.getPath2ResultFile(), 
      function(data){
        response.write(data);
        response.end();
      });  
    ...
    response.write("Algo processing completed with no output file");
    response.end();
  } 
}
